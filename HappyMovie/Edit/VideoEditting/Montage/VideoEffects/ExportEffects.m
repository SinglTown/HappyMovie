...
//    NSString *nowTimeStr = [formatter stringFromDate:[NSDate dateWithTimeIntervalSinceNow:0]];
    ...
    NSString *fileName = [[DataStore sharedDataStore] saveFileToData];
    
    //插入到数据库
    [[DataStore sharedDataStore] insertUrl:fileName];
        ...
        [library writeVideoAtPathToSavedPhotosAlbum:exportURL completionBlock:^(NSURL *assetURL, NSError *error)
         {
             ...
             if (!error)
             {
                  message = GBLocalizedString(@"成功保存到相册");
                 
                 // NSLog(@"保存到相册之后的URL-----%@",exportURL);
             }
             else
             {
...
//             NSLog(@"%@", message);
    ...
    {
        NSString *message = GBLocalizedString(@"保存相册失败");;
       // NSLog(@"%@", message);
